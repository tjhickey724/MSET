...
document.getElementById('go').addEventListener('click',function(event){
  ...
  let log2TotalEdits = parseInt(document.getElementById('totalEditsTF').value)
  let totalEdits = Math.pow(2,log2TotalEdits)
  ...
  runTimeTests(k,numEdits,totalEdits,numLists,initSize,burstSize,shuffled)
})


function runTimeTests(k0,numEdits0, totalEdits, numLists0,initSize0,burstSize0,shuffled){
  ...
  for(let j0=1;j0<=k0;(j0++,j*=2)){
    ...
    let repeats = totalEdits/numEdits
    console.log(JSON.stringify([repeats,numEdits,numLists,initSize,burstSize,testVal,j,multiplier,]))
    window.avlReset()
    ...
    for (let i=0; i<repeats;i++){
      lists = runTestsA(numEdits,numLists,initSize,burstSize,shuffled);      
    }
    let b = performance.now();
    //checkEquality(lists)

    let totalTime = Math.round(1000*(b-a))/repeats // in microseconds
    ...
    */
    let s =(`numEdits=${numEdits} numLists=${numLists} initSize=${initSize} burstSize=${burstSize}`+
      ` j=${j} numGCs=${numGCs} treeHeight=${treeHeight} time (sec)=${Math.round(totalTime/1000000)} `+
